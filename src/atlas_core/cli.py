    ...
    if getattr(args, "schema", False) and \
        ...
        metrics_schema: dict[str, Any] = {
            ...
            "notes": [
                ...
                "SPEC 169: --alert-window MINUTES zaman-pencereli alert.",
                "SPEC 175: alert_options + alert_payload alanları JSON şemada.",
            ],
            # SPEC 175: alert alt-ailesi CLI seçenekleri (schema kullanıcısı
            # koddan değil şemadan öğrenmeli). Prometheus çıktısına EKLENMEDİ
            # (YAGNI — SPEC 164 kalıbı; JSON şeması daha zengin).
            "alert_options": [
                {"name": "--alert PCT", "spec": "029",
                 "desc": "cache-hit oranı PCT altı → stderr UYARI + exit 8"},
                {"name": "--alert-window MINUTES", "spec": "169",
                 "desc": "--alert için ayrı zaman penceresi (tail/--window bağımsız)"},
                {"name": "--alert-email", "spec": "059",
                 "desc": "SMTP notify (env: ATLAS_SMTP_*)"},
                {"name": "--alert-webhook URL", "spec": "064",
                 "desc": "POST JSON webhook (Slack/Discord/Teams uyumlu)"},
                {"name": "--alert-slack URL", "spec": "068",
                 "desc": "Slack incoming webhook özel `{text}` formatı"},
                {"name": "--alert-history [PATH]", "spec": "126",
                 "desc": "NDJSON append log (default .atlas/alert-history.jsonl)"},
                {"name": "--alert-history-show [PATH]", "spec": "132",
                 "desc": "NDJSON log OKU (metrics özet yerine)"},
            ],
            # SPEC 175: alert-history NDJSON + webhook payload alanları.
            # `when` = alanın hangi koşulda yazıldığı ("always" veya
            # tetikleyen bayrak). SPEC 032.4 alan-ekleme bit-uyumlu.
            "alert_payload": [
                {"name": "ts", "type": "str (ISO 8601)",
                 "when": "always (history)", "spec": "126"},
                {"name": "alert", "type": "str",
                 "when": "always", "spec": "064"},
                ...
                {"name": "message", "type": "str",
                 "when": "webhook only (SPEC 064)", "spec": "064"},
                {"name": "channels", "type": "list[str]",
                 "when": "history only (SPEC 126)", "spec": "126"},
                {"name": "alert_window_minutes", "type": "float",
                 "when": "yalnız --alert-window verildiğinde", "spec": "169"},
